...
--
-- Host: 127.0.0.1
-- Erstellungszeit: 22. Dez 2022 um 17:35
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `lehrkraft_informationen`  AS SELECT `tbl_lehrkraft`.`Vorname` AS `Vorname`, `tbl_lehrkraft`.`Nachname` AS `Nachname`, `tbl_lehrkraft`.`Kuerzel` AS `Kuerzel`, `tbl_lehrkraft`.`E-Mail` AS `E-Mail` FROM `tbl_lehrkraft``tbl_lehrkraft`  ;
...
--
ALTER TABLE `tbl_benutzer`
  MODIFY `PK_benutzer` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
...
--
ALTER TABLE `tbl_termin`
  ADD CONSTRAINT `tbl_termin_ibfk_1` FOREIGN KEY (`FK_lehrkraft`) REFERENCES `tbl_lehrkraft` (`PK_lehrkraft`),
  ADD CONSTRAINT `tbl_termin_ibfk_2` FOREIGN KEY (`FK_ansprechpartner`) REFERENCES `tbl_ansprechpartner` (`PK_ansprechpartner`),
  ADD CONSTRAINT `tbl_termin_ibfk_3` FOREIGN KEY (`FK_veranstaltung`) REFERENCES `tbl_veranstaltung` (`PK_veranstaltung`);
